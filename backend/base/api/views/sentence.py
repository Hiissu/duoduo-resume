...
class SentencesGetView(APIView):
        ...
        try:
            ...
            if language_code in language_list:
                file_path = settings.BASE_DIR / \
                    f"documents/sentences/{language_code}_sentences.json"
                return FileResponse(open(file_path, "rb"))
            return Response({"message": "Language code does not exist"}, status=status.HTTP_400_BAD_REQUEST)
        except:
            return Response({"message": "Something went wrong when retrieving sentences"}, status=status.HTTP_400_BAD_REQUEST)
